/**
 * GastosComponent — módulo "Gastos e Ingresos" migrado a la nueva
 * arquitectura (Fase 2, módulo 9).
 *
 * Diferencias clave vs modules/gastos.js legacy:
 *  - Component (mount/update/unmount) en vez de `App.register` + string HTML.
 *  - Interactividad por EVENT DELEGATION (data-action) con TRES listeners
 *    delegados en document: 'click' (tabs, chips, botones, modales),
 *    'input' (buscador) y 'change' (el <input type="file"> del comprobante —
 *    igual que el legacy, que usa `onchange`, no `oninput`, para disparar el
 *    FileReader del preview).
 *  - Lectura vía AppStore (gastos, fondos) y mutación vía
 *    AppStore.registrarGasto / actualizarGasto / eliminarGasto /
 *    registrarIngreso / eliminarFondoMovimiento — mismos efectos y eventos
 *    que el legacy con DB.*. Se suscribe tanto a 'gastos:update' como a
 *    'fondos:update' (el legacy también escucha ambos: un gasto nuevo cambia
 *    el balance de fondos y viceversa).
 *  - La subida del comprobante (POST /gastos/<id>/comprobante) usa un
 *    GastosRepository propio construido con el ApiClient inyectado (mismo
 *    patrón que Alimentación/Almacén), cuyo método `subirComprobante()`
 *    llama a `api.postForm()` — éste ya agrega el header Authorization que
 *    el legacy omitía (bug corregido en el ApiClient nuevo; ver comentario en
 *    api-client.ts). El preview local del comprobante ANTES de subir
 *    (FileReader) es 100% local y no toca el backend, igual que el legacy.
 *  - Como el AppStore no expone un método de "solo actualizar el campo
 *    comprobante en caché", tras subir el archivo se muta el gasto cacheado
 *    (`store.gastos` es la misma referencia de array que usa el AppStore) y
 *    se dispara `store.emit('gastos:update')` — `emit` ya es parte de la
 *    superficie pública del AppStore (bus de eventos), así que esto no
 *    requiere tocar app-store.ts.
 *  - Decisión de diseño (gastos "Auto-almacén", fuenteAuto === 'compra_almacen'):
 *    el legacy los muestra con una insignia distinta ("Auto·Almacén" vs
 *    "Manual") pero NO deshabilita sus botones de editar/eliminar — están
 *    sujetos a los mismos permisos (Auth.canWrite('gastos')) que un gasto
 *    manual. Se replica ese comportamiento tal cual, sin agregar
 *    restricciones nuevas que el legacy no tenía.
 *  - esc() en todo dato que viene de la base de datos.
 */
import { Component, resolveApiBase } from '@core/index';
import type { ApiClient } from '@core/index';
import type { AppStore } from '@store/app-store';
import { GastosRepository } from '@domain/gastos/gastos.repository';
import type { Gasto } from '@domain/gastos/gastos.types';
import type { FondoMov } from '@domain/fondos/fondos.types';
import { diaMes } from '@domain/shared/meses';
import { Auth } from '@shell/auth';
import { esc, toast, modal, closeModal } from '@shell/ui';

type Tab = 'egresos' | 'ingresos';

const CAT_COLORES: Record<string, { bg: string; fg: string }> = {
  Alimentos: { bg: '#DDEDF1', fg: '#1C6678' },
  Almacén: { bg: '#E0F0FF', fg: '#015a9e' },
  Regalos: { bg: '#EDE7FD', fg: '#6B4EEA' },
  Útiles: { bg: '#FDF2D5', fg: '#9A6B0A' },
  Servicios: { bg: '#e8e8e8', fg: '#555' },
  Transporte: { bg: '#E1EDFD', fg: '#2A5FA0' },
  Higiene: { bg: '#E8F7F1', fg: '#1D7A56' },
  Otros: { bg: '#e8e8e8', fg: '#6E7872' },
};

const TIPO_INGRESO: Record<string, { bg: string; fg: string }> = {
  'Donación de dinero': { bg: '#edfde0', fg: '#3d8a20' },
  Subvención: { bg: '#E0F0FF', fg: '#015a9e' },
  Evento: { bg: '#EDE7FD', fg: '#6B4EEA' },
  Colecta: { bg: '#FDF2D5', fg: '#9A6B0A' },
  Transferencia: { bg: '#E8F7F1', fg: '#1D7A56' },
  'Otro ingreso': { bg: '#e8e8e8', fg: '#555' },
};

const CATEGORIAS = Object.keys(CAT_COLORES);

export class GastosComponent extends Component {
  private readonly repo: GastosRepository;
  private readonly apiBase = resolveApiBase();
  private tab: Tab = 'egresos';
  private search = '';
  private catFiltro = 'todas';
  private readonly unsubs: Array<() => void> = [];
  private readonly onClick = (e: Event) => this.handleClick(e);
  private readonly onInput = (e: Event) => this.handleInput(e);
  private readonly onChange = (e: Event) => this.handleChange(e);

  constructor(private readonly store: AppStore, api: ApiClient) {
    super();
    this.repo = new GastosRepository(api);
  }

  protected override onMount(): void {
    document.addEventListener('click', this.onClick);
    document.addEventListener('input', this.onInput);
    document.addEventListener('change', this.onChange);
    for (const evento of ['gastos:update', 'fondos:update'] as const) {
      this.unsubs.push(this.store.on(evento, () => this.update()));
    }
  }

  protected override onUnmount(): void {
    document.removeEventListener('click', this.onClick);
    document.removeEventListener('input', this.onInput);
    document.removeEventListener('change', this.onChange);
    for (const unsub of this.unsubs) unsub();
    this.unsubs.length = 0;
  }

  /* ---------- Delegación de eventos ---------- */
  private handleClick(e: Event): void {
    const target = (e.target as HTMLElement)?.closest<HTMLElement>('[data-action]');
    if (!target) return;
    const action = target.dataset.action;
    const id = target.dataset.id ? Number(target.dataset.id) : undefined;
    switch (action) {
      case 'abrir-gasto': this.abrirGasto(); break;
      case 'editar-gasto': if (id != null) this.editarGasto(id); break;
      case 'guardar-gasto': void this.guardarGasto(); break;
      case 'guardar-edicion-gasto': if (id != null) void this.guardarEdicionGasto(id); break;
      case 'abrir-ingreso': this.abrirIngreso(); break;
      case 'guardar-ingreso': void this.guardarIngreso(); break;
      case 'cerrar-modal': closeModal(); break;
      case 'set-tab': if (target.dataset.tab) this.setTab(target.dataset.tab as Tab); break;
      case 'set-cat': if (target.dataset.cat) this.setCat(target.dataset.cat); break;
      case 'click-file-input': {
        const targetId = target.dataset.target;
        if (targetId) document.getElementById(targetId)?.click();
        break;
      }
      case 'ver-comprobante': if (id != null) this.verComprobante(id); break;
      case 'subir-comprobante': if (id != null) this.subirComprobante(id); break;
      case 'confirmar-eliminar-gasto': if (id != null) this.confirmarEliminarGasto(id); break;
      case 'do-eliminar-gasto': if (id != null) void this.doEliminarGasto(id); break;
      case 'confirmar-eliminar-ingreso': if (id != null) this.confirmarEliminarIngreso(id); break;
      case 'do-eliminar-ingreso': if (id != null) void this.doEliminarIngreso(id); break;
    }
  }

  private handleInput(e: Event): void {
    const target = (e.target as HTMLElement)?.closest<HTMLElement>('[data-action]');
    if (!target) return;
    if (target.dataset.action === 'buscar') this.setSearch((target as HTMLInputElement).value);
  }

  private handleChange(e: Event): void {
    const target = (e.target as HTMLElement)?.closest<HTMLElement>('[data-action]');
    if (!target) return;
    if (target.dataset.action === 'preview-comprobante') this.previewComprobante(target as HTMLInputElement);
  }

  private setTab(t: Tab): void {
    this.tab = t;
    this.search = '';
    this.catFiltro = 'todas';
    this.update();
  }

  private setSearch(v: string): void {
    this.search = v;
    this.update();
  }

  private setCat(c: string): void {
    this.catFiltro = c;
    this.update();
  }

  /* ---------- RENDER PRINCIPAL ---------- */
  protected render(): string {
    const f = this.store.fondos;
    const balPos = f.balance >= 0;
    const balCol = balPos ? '#1D7A56' : '#C24A30';
    const balBg = balPos ? '#E8F7F1' : '#FDE7E1';

    const gastos = this.store.gastos.filter((g) => {
      if (this.catFiltro !== 'todas' && g.categoria !== this.catFiltro) return false;
      if (this.search) {
        const q = this.search.toLowerCase();
        return g.categoria.toLowerCase().includes(q) ||
          g.proveedor.toLowerCase().includes(q) ||
          (g.observacion || '').toLowerCase().includes(q);
      }
      return true;
    });

    const totalGastos = this.store.gastos.reduce((s, g) => s + g.monto, 0);
    const conComprobante = this.store.gastos.filter((g) => g.comprobante).length;
    const autoGenerados = this.store.gastos.filter((g) => g.fuenteAuto === 'compra_almacen').length;

    const ingresos = (f.movimientos || []).filter((m) => m.tipo === 'ingreso');
    const totalIngresos = ingresos.reduce((s, m) => s + m.monto, 0);

    const puedeEscribir = Auth.canWrite('gastos');

    return `
    <div class="page-header">
      <div>
        <h1>Gastos e Ingresos</h1>
        <p>Transparencia de fondos · comprobantes adjuntos · balance en tiempo real</p>
      </div>
      ${puedeEscribir ? `
      <div style="margin-left:auto;display:flex;gap:10px;">
        <button class="btn btn-outline" data-action="abrir-ingreso" style="border-color:#1D7A56;color:#1D7A56;">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
          Registrar ingreso
        </button>
        <button class="btn btn-primary" data-action="abrir-gasto">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 5v14M5 12h14"/></svg>
          Registrar gasto
        </button>
      </div>` : `<div style="margin-left:auto;"><span style="font-size:12px;background:var(--line);padding:5px 12px;border-radius:20px;color:var(--muted);">Solo lectura</span></div>`}
    </div>

    <!-- BALANCE -->
    <div style="background:${balBg};border:1.5px solid ${balCol}44;border-radius:16px;padding:20px 24px;margin-bottom:18px;display:flex;align-items:center;gap:24px;flex-wrap:wrap;">
      <div style="flex:1;min-width:180px;">
        <div style="font-size:11px;font-weight:700;letter-spacing:.6px;color:${balCol};text-transform:uppercase;margin-bottom:4px;">
          ${balPos ? '▲' : '▼'} Fondos disponibles
        </div>
        <div style="font-size:38px;font-weight:800;color:${balCol};line-height:1;">
          S/ ${Math.abs(f.balance).toLocaleString('es-PE', { minimumFractionDigits: 2 })}
        </div>
        <div style="font-size:12.5px;color:${balCol};opacity:.7;margin-top:4px;">
          ${balPos ? 'Balance positivo' : '⚠ Egresos superan ingresos'}
        </div>
      </div>
      <div style="display:flex;gap:12px;flex-wrap:wrap;">
        <div style="background:#fff8;border-radius:12px;padding:12px 18px;min-width:130px;">
          <div style="font-size:11px;color:var(--muted);font-weight:700;letter-spacing:.4px;text-transform:uppercase;">Ingresos totales</div>
          <div style="font-size:22px;font-weight:800;color:#1D7A56;margin-top:3px;">
            S/ ${f.ingresos.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
          </div>
        </div>
        <div style="background:#fff8;border-radius:12px;padding:12px 18px;min-width:130px;">
          <div style="font-size:11px;color:var(--muted);font-weight:700;letter-spacing:.4px;text-transform:uppercase;">Egresos totales</div>
          <div style="font-size:22px;font-weight:800;color:#C24A30;margin-top:3px;">
            S/ ${f.egresos.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
          </div>
        </div>
      </div>
    </div>

    <!-- TABS -->
    <div style="display:flex;gap:4px;margin-bottom:16px;border-bottom:2px solid var(--line);padding-bottom:0;">
      ${([['egresos', 'Egresos / Gastos'], ['ingresos', 'Ingresos']] as Array<[Tab, string]>).map(([key, label]) => `
        <button data-action="set-tab" data-tab="${key}"
          style="padding:9px 20px;border:none;background:none;font-size:14px;font-weight:700;cursor:pointer;
            border-bottom:3px solid ${this.tab === key ? 'var(--primary)' : 'transparent'};
            color:${this.tab === key ? 'var(--primary)' : 'var(--muted)'};margin-bottom:-2px;">
          ${label}
          <span style="margin-left:6px;background:${this.tab === key ? 'var(--primary)' : 'var(--line)'};color:${this.tab === key ? '#fff' : 'var(--muted)'};border-radius:20px;padding:1px 8px;font-size:11px;font-weight:700;">
            ${key === 'egresos' ? this.store.gastos.length : ingresos.length}
          </span>
        </button>`).join('')}
    </div>

    ${this.tab === 'egresos'
      ? this.renderEgresos(gastos, totalGastos, conComprobante, autoGenerados, puedeEscribir)
      : this.renderIngresos(ingresos, totalIngresos, puedeEscribir)}
    `;
  }

  private renderEgresos(gastos: Gasto[], total: number, conComp: number, autoGen: number, puedeEscribir: boolean): string {
    return `
    <div class="kpi-grid cols-3" style="margin-bottom:16px;">
      <div class="kpi-card">
        <div class="label">Total egresos</div>
        <div class="value" style="font-size:28px;">S/ ${total.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</div>
      </div>
      <div class="kpi-card">
        <div class="label">Con comprobante</div>
        <div class="value" style="font-size:28px;color:var(--success);">${conComp} <span style="font-size:16px;color:var(--muted);">/ ${this.store.gastos.length}</span></div>
        <div class="sub">${this.store.gastos.length > 0 ? Math.round((conComp / this.store.gastos.length) * 100) + '% documentados' : '—'}</div>
      </div>
      <div class="kpi-card">
        <div class="label">Auto-generados (compras)</div>
        <div class="value" style="font-size:28px;color:var(--primary);">${autoGen}</div>
        <div class="sub" style="color:var(--muted);">desde módulo almacén</div>
      </div>
    </div>

    <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-wrap:wrap;">
      <div style="position:relative;flex:1;min-width:200px;">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--faint);pointer-events:none;"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input type="text" placeholder="Buscar gasto…" value="${esc(this.search)}" data-action="buscar"
          style="width:100%;padding:8px 12px 8px 30px;border:1.5px solid var(--border);border-radius:9px;font-size:13px;">
      </div>
      <button class="filter-chip ${this.catFiltro === 'todas' ? 'active' : ''}" data-action="set-cat" data-cat="todas">Todas</button>
      ${CATEGORIAS.map((c) => `<button class="filter-chip ${this.catFiltro === c ? 'active' : ''}" data-action="set-cat" data-cat="${esc(c)}">${esc(c)}</button>`).join('')}
    </div>

    <div class="table-card">
      ${gastos.length ? `
      <div class="table-head" style="grid-template-columns:80px 1.1fr 1fr 1.4fr 1.3fr 90px 110px;">
        <span>Fecha</span><span>Categoría</span><span>Monto</span><span>Proveedor</span><span>Nota</span><span>Origen</span><span>Acciones</span>
      </div>
      ${gastos.map((g) => `
        <div class="table-row" style="grid-template-columns:80px 1.1fr 1fr 1.4fr 1.3fr 90px 110px;">
          <span style="font-size:12.5px;color:var(--muted);">${esc(g.fecha)}</span>
          <div style="background:${esc(g.catBg)};color:${esc(g.catFg)};border-radius:20px;padding:4px 10px;font-size:12px;font-weight:700;display:inline-flex;align-items:center;">${esc(g.categoria)}</div>
          <span style="font-size:14px;font-weight:800;">S/ ${g.monto.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</span>
          <span style="font-size:13px;font-weight:600;">${esc(g.proveedor)}</span>
          <span style="font-size:12.5px;color:var(--muted);font-style:${g.observacion ? 'normal' : 'italic'};">${esc(g.observacion) || '—'}</span>
          <div>
            ${g.fuenteAuto === 'compra_almacen'
              ? `<span style="background:#E0F0FF;color:#015a9e;border-radius:20px;padding:3px 8px;font-size:10.5px;font-weight:700;">Auto·Almacén</span>`
              : `<span style="background:var(--line);color:var(--muted);border-radius:20px;padding:3px 8px;font-size:10.5px;font-weight:700;">Manual</span>`}
          </div>
          <div style="display:flex;align-items:center;gap:4px;">
            ${g.comprobante
              ? `<button class="btn btn-sm btn-outline" style="padding:5px 7px;" data-action="ver-comprobante" data-id="${g.id}" title="Ver comprobante">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--success)" stroke-width="2.2"><path d="M20 6 9 17l-5-5"/></svg>
                </button>`
              : `<button class="btn btn-sm btn-outline" style="padding:5px 7px;" data-action="subir-comprobante" data-id="${g.id}" title="Adjuntar comprobante">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
                </button>`}
            ${puedeEscribir ? `
            <button class="btn btn-sm btn-outline" style="padding:5px 7px;" data-action="editar-gasto" data-id="${g.id}" title="Editar">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4Z"/></svg>
            </button>
            <button class="btn btn-sm btn-outline" style="padding:5px 7px;color:var(--danger);border-color:var(--danger)20;" data-action="confirmar-eliminar-gasto" data-id="${g.id}" title="Eliminar">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>
            </button>` : ''}
          </div>
        </div>`).join('')}
      ` : `<div style="padding:36px;text-align:center;color:var(--muted);font-size:14px;">Sin gastos registrados${this.search ? ' con ese criterio' : ''}.</div>`}
    </div>`;
  }

  private renderIngresos(ingresos: FondoMov[], total: number, puedeEscribir: boolean): string {
    return `
    <div class="kpi-grid cols-2" style="margin-bottom:16px;">
      <div class="kpi-card">
        <div class="label">Total ingresos</div>
        <div class="value" style="font-size:28px;color:#1D7A56;">S/ ${total.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</div>
      </div>
      <div class="kpi-card">
        <div class="label">Cantidad de ingresos</div>
        <div class="value" style="font-size:28px;">${ingresos.length}</div>
      </div>
    </div>

    <div class="table-card">
      ${ingresos.length ? `
      <div class="table-head" style="grid-template-columns:80px 1.2fr 1fr 1.5fr 70px;">
        <span>Fecha</span><span>Tipo</span><span>Monto</span><span>Descripción / Donante</span><span></span>
      </div>
      ${ingresos.map((m) => {
        const c = TIPO_INGRESO[m.categoria] || TIPO_INGRESO['Otro ingreso'];
        return `
        <div class="table-row" style="grid-template-columns:80px 1.2fr 1fr 1.5fr 70px;">
          <span style="font-size:12.5px;color:var(--muted);">${esc(m.fecha)}</span>
          <div style="background:${c.bg};color:${c.fg};border-radius:20px;padding:4px 10px;font-size:12px;font-weight:700;display:inline-flex;">${esc(m.categoria) || 'Ingreso'}</div>
          <span style="font-size:14px;font-weight:800;color:#1D7A56;">+ S/ ${m.monto.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</span>
          <span style="font-size:13px;color:var(--muted);">${esc(m.descripcion) || '—'}</span>
          ${puedeEscribir ? `
          <button class="btn btn-sm btn-outline" style="padding:5px 7px;color:var(--danger);border-color:var(--danger)20;" data-action="confirmar-eliminar-ingreso" data-id="${m.id}" title="Eliminar">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>
          </button>` : ''}
        </div>`;
      }).join('')}
      ` : `<div style="padding:36px;text-align:center;color:var(--muted);font-size:14px;">Sin ingresos registrados aún.</div>`}
    </div>`;
  }

  /* ---------- MODAL GASTO ---------- */
  private abrirGasto(): void {
    modal(`
      <h2>Registrar gasto</h2>
      <div class="form-grid">
        <div class="form-group">
          <label>Fecha *</label>
          <input type="date" id="g-fecha" value="${new Date().toISOString().split('T')[0]}">
        </div>
        <div class="form-group">
          <label>Categoría *</label>
          <select id="g-cat">${CATEGORIAS.map((c) => `<option>${esc(c)}</option>`).join('')}</select>
        </div>
      </div>
      <div class="form-grid">
        <div class="form-group">
          <label>Monto (S/) *</label>
          <input type="number" id="g-monto" placeholder="0.00" step="0.01" min="0">
        </div>
        <div class="form-group">
          <label>Proveedor / A quién se pagó *</label>
          <input type="text" id="g-prov" placeholder="Nombre del proveedor">
        </div>
      </div>
      <div class="form-group">
        <label>Nota / Observación <span style="font-weight:400;color:var(--faint);">opcional</span></label>
        <input type="text" id="g-obs" placeholder="Ej: Compra mensual de alimentos">
      </div>

      <!-- COMPROBANTE -->
      <div class="ficha-section" style="margin-top:4px;">
        <div class="ficha-section-title">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 3v5h5M5 21V4a1 1 0 0 1 1-1h8l5 5v13a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1Z"/></svg>
          Comprobante
          <span style="font-size:11px;font-weight:400;color:var(--faint);margin-left:4px;">foto de factura, recibo o ticket</span>
        </div>
        <div id="g-comp-wrap" data-action="click-file-input" data-target="g-comp-input"
          style="border:2px dashed var(--border);border-radius:12px;padding:24px;text-align:center;cursor:pointer;background:var(--bg);transition:border .15s;">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--faint)" stroke-width="1.5" style="margin-bottom:8px;"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
          <div style="font-size:13px;font-weight:600;color:var(--muted);">Hacer clic para adjuntar</div>
          <div style="font-size:12px;color:var(--faint);margin-top:3px;">JPG, PNG o PDF · máx. 5MB</div>
        </div>
        <input type="file" id="g-comp-input" accept="image/*,.pdf" style="display:none" data-action="preview-comprobante">
      </div>

      <div class="modal-footer">
        <button class="btn btn-outline" data-action="cerrar-modal">Cancelar</button>
        <button class="btn btn-primary" data-action="guardar-gasto">Registrar gasto</button>
      </div>`);
  }

  /** Preview 100% local (FileReader) — no toca el backend. Tras elegir un
   *  archivo, se quita data-action del wrap (equivalente a wrap.onclick=null
   *  en el legacy) para no reabrir el selector con cada clic sobre el preview. */
  private previewComprobante(input: HTMLInputElement): void {
    const file = input.files?.[0];
    const wrap = document.getElementById('g-comp-wrap');
    if (!file || !wrap) return;
    if (file.type.startsWith('image/')) {
      const reader = new FileReader();
      reader.onload = (e) => {
        wrap.innerHTML = `
          <img src="${e.target?.result}" style="max-height:160px;border-radius:8px;object-fit:contain;">
          <div style="font-size:12px;color:var(--success);margin-top:8px;font-weight:600;">✓ ${esc(file.name)}</div>`;
      };
      reader.readAsDataURL(file);
    } else {
      wrap.innerHTML = `
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.5" style="margin-bottom:6px;"><path d="M14 3v5h5M5 21V4a1 1 0 0 1 1-1h8l5 5v13a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1Z"/></svg>
        <div style="font-size:13px;font-weight:600;color:var(--primary);">${esc(file.name)}</div>
        <div style="font-size:12px;color:var(--success);margin-top:4px;font-weight:600;">✓ PDF adjunto</div>`;
    }
    wrap.style.borderColor = 'var(--success)';
    wrap.removeAttribute('data-action');
  }

  private async guardarGasto(): Promise<void> {
    const fecha = this.val('g-fecha');
    const cat = this.val('g-cat');
    const monto = parseFloat(this.val('g-monto') || '0');
    const prov = this.val('g-prov').trim();
    const obs = this.val('g-obs').trim();
    if (!monto || monto <= 0) { toast('Ingresa un monto válido', 'error'); return; }
    if (!prov) { toast('Ingresa el proveedor', 'error'); return; }

    const c = CAT_COLORES[cat] || CAT_COLORES.Otros;
    const btn = document.querySelector<HTMLButtonElement>('.modal .btn-primary');
    if (btn) { btn.disabled = true; btn.textContent = 'Guardando…'; }

    const gasto: Gasto = {
      id: 0, fecha: '', fechaISO: fecha, categoria: cat, monto,
      proveedor: prov, fondo: 'Fondo General', observacion: obs,
      comprobante: '', fuenteAuto: '', catBg: c.bg, catFg: c.fg,
    };
    const res = await this.store.registrarGasto(gasto);

    const fileInput = document.getElementById('g-comp-input') as HTMLInputElement | null;
    const file = fileInput?.files?.[0];
    let conComprobante = false;
    if (file) {
      try {
        const up = await this.repo.subirComprobante(res.id, file);
        if (up && up.ok && up.url) {
          const g = this.store.gastos.find((x) => x.id === res.id);
          if (g) g.comprobante = up.url;
          this.store.emit('gastos:update');
          conComprobante = true;
        }
      } catch {
        /* upload falló silenciosamente, igual que el legacy */
      }
    }

    closeModal();
    toast(`Gasto de S/${monto.toFixed(2)} registrado${conComprobante ? ' con comprobante' : ''}`, 'success');
  }

  /* ---------- EDITAR GASTO ---------- */
  private editarGasto(id: number): void {
    const g = this.store.gastos.find((x) => x.id === id);
    if (!g) return;
    modal(`
      <h2>Editar gasto</h2>
      <div class="form-grid">
        <div class="form-group">
          <label>Fecha *</label>
          <input type="date" id="ge-fecha" value="${esc(g.fechaISO || '')}">
        </div>
        <div class="form-group">
          <label>Categoría *</label>
          <select id="ge-cat">${CATEGORIAS.map((c) => `<option ${c === g.categoria ? 'selected' : ''}>${esc(c)}</option>`).join('')}</select>
        </div>
      </div>
      <div class="form-grid">
        <div class="form-group">
          <label>Monto (S/) *</label>
          <input type="number" id="ge-monto" value="${g.monto}" step="0.01" min="0">
        </div>
        <div class="form-group">
          <label>Proveedor *</label>
          <input type="text" id="ge-prov" value="${esc(g.proveedor)}">
        </div>
      </div>
      <div class="form-group">
        <label>Nota / Observación</label>
        <input type="text" id="ge-obs" value="${esc(g.observacion)}">
      </div>
      <div class="modal-footer">
        <button class="btn btn-outline" data-action="cerrar-modal">Cancelar</button>
        <button class="btn btn-primary" data-action="guardar-edicion-gasto" data-id="${id}">Guardar cambios</button>
      </div>`);
  }

  private async guardarEdicionGasto(id: number): Promise<void> {
    const fecha = this.val('ge-fecha');
    const cat = this.val('ge-cat');
    const monto = parseFloat(this.val('ge-monto') || '0');
    const prov = this.val('ge-prov').trim();
    const obs = this.val('ge-obs').trim();
    if (!monto || monto <= 0) { toast('Monto inválido', 'error'); return; }
    if (!prov) { toast('Proveedor obligatorio', 'error'); return; }
    const c = CAT_COLORES[cat] || CAT_COLORES.Otros;
    const btn = document.querySelector<HTMLButtonElement>('.modal .btn-primary');
    if (btn) { btn.disabled = true; btn.textContent = 'Guardando…'; }

    const res = await this.store.actualizarGasto(id, {
      fechaISO: fecha,
      fecha: diaMes(new Date(fecha + 'T00:00:00')),
      categoria: cat, monto, proveedor: prov, observacion: obs,
      catBg: c.bg, catFg: c.fg,
    }) as { ok?: boolean } | null;

    if (res && res.ok === false) {
      toast('Error al guardar', 'error');
      if (btn) { btn.disabled = false; btn.textContent = 'Guardar cambios'; }
      return;
    }
    closeModal();
    toast('Gasto actualizado', 'success');
  }

  /* ---------- MODAL INGRESO ---------- */
  private abrirIngreso(): void {
    modal(`
      <h2>Registrar ingreso</h2>
      <p style="margin:-4px 0 16px;font-size:13px;color:var(--muted);">Donaciones de dinero, polladas, colectas, subvenciones, etc.</p>

      <div class="form-grid">
        <div class="form-group">
          <label>Fecha *</label>
          <input type="date" id="ing-fecha" value="${new Date().toISOString().split('T')[0]}">
        </div>
        <div class="form-group">
          <label>Tipo de ingreso *</label>
          <select id="ing-tipo">
            ${Object.keys(TIPO_INGRESO).map((t) => `<option>${esc(t)}</option>`).join('')}
          </select>
        </div>
      </div>
      <div class="form-group">
        <label>Monto (S/) *</label>
        <input type="number" id="ing-monto" placeholder="0.00" step="0.01" min="0.01">
      </div>
      <div class="form-group">
        <label>Descripción / Donante / Evento</label>
        <input type="text" id="ing-desc" placeholder="Ej: Donación de la parroquia San José · Pollada 5 julio">
      </div>
      <div class="modal-footer">
        <button class="btn btn-outline" data-action="cerrar-modal">Cancelar</button>
        <button class="btn btn-primary" style="background:#1D7A56;border-color:#1D7A56;" data-action="guardar-ingreso">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
          Registrar ingreso
        </button>
      </div>`, { narrow: true });
  }

  private async guardarIngreso(): Promise<void> {
    const monto = parseFloat(this.val('ing-monto') || '0');
    const tipo = this.val('ing-tipo');
    const desc = this.val('ing-desc').trim();
    const fecha = this.val('ing-fecha');
    if (!monto || monto <= 0) { toast('Ingresa un monto válido', 'error'); return; }

    const btn = document.querySelector<HTMLButtonElement>('.modal .btn-primary');
    if (btn) { btn.disabled = true; btn.textContent = 'Guardando…'; }

    const res = await this.store.registrarIngreso({
      monto, descripcion: desc, categoria: tipo, fuente: 'donacion', fecha,
    }) as { error?: string } | null;

    if (res && res.error) {
      toast(res.error, 'error');
      if (btn) { btn.disabled = false; btn.textContent = 'Registrar ingreso'; }
      return;
    }
    closeModal();
    toast(`Ingreso de S/${monto.toLocaleString('es-PE', { minimumFractionDigits: 2 })} registrado`, 'success');
  }

  /* ---------- COMPROBANTE ---------- */
  private verComprobante(id: number): void {
    const g = this.store.gastos.find((x) => x.id === id);
    if (!g) return;
    const url = g.comprobante ? this.apiBase + g.comprobante : null;
    const esImg = url ? /\.(jpg|jpeg|png|gif|webp)$/i.test(url) : false;
    modal(`
      <h2>Comprobante · ${esc(g.categoria)}</h2>
      <div style="background:var(--bg);border-radius:12px;padding:16px;margin-bottom:14px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div><div style="font-size:11px;color:var(--muted);font-weight:700;margin-bottom:3px;">FECHA</div><div style="font-weight:600;">${esc(g.fecha)}</div></div>
        <div><div style="font-size:11px;color:var(--muted);font-weight:700;margin-bottom:3px;">MONTO</div><div style="font-weight:800;font-size:18px;">S/ ${g.monto.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</div></div>
        <div><div style="font-size:11px;color:var(--muted);font-weight:700;margin-bottom:3px;">PROVEEDOR</div><div style="font-weight:600;">${esc(g.proveedor)}</div></div>
        <div><div style="font-size:11px;color:var(--muted);font-weight:700;margin-bottom:3px;">CATEGORÍA</div><div style="font-weight:600;">${esc(g.categoria)}</div></div>
        ${g.observacion ? `<div style="grid-column:span 2;"><div style="font-size:11px;color:var(--muted);font-weight:700;margin-bottom:3px;">NOTA</div><div>${esc(g.observacion)}</div></div>` : ''}
      </div>
      ${url
        ? esImg
          ? `<img src="${esc(url)}" style="width:100%;border-radius:10px;max-height:340px;object-fit:contain;background:#000;">`
          : `<div style="text-align:center;padding:24px;border:1.5px solid var(--border);border-radius:12px;">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.5" style="margin-bottom:8px;"><path d="M14 3v5h5M5 21V4a1 1 0 0 1 1-1h8l5 5v13a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1Z"/></svg>
              <div style="font-weight:600;margin-bottom:8px;">Comprobante PDF adjunto</div>
              <a href="${esc(url)}" target="_blank" class="btn btn-primary" style="text-decoration:none;">Abrir PDF</a>
            </div>`
        : `<div style="background:#FDF2D5;border-radius:10px;padding:14px 16px;font-size:13px;color:#9A6B0A;text-align:center;">
            Sin comprobante adjunto.
            <button class="btn btn-sm btn-outline" style="margin-left:10px;" data-action="subir-comprobante" data-id="${g.id}">Adjuntar ahora</button>
          </div>`}
      <div class="modal-footer"><button class="btn btn-outline" data-action="cerrar-modal">Cerrar</button></div>
    `, { narrow: true });
  }

  /** Abre un selector de archivo "suelto" (sin modal propio), igual que el
   *  legacy `subirComprobante(id)`, ya sea desde la tabla o desde el modal de
   *  "ver comprobante" cuando aún no hay uno adjunto. */
  private subirComprobante(id: number): void {
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = 'image/*,.pdf';
    input.onchange = async () => {
      const file = input.files?.[0];
      if (!file) return;
      toast('Subiendo comprobante…', 'info');
      try {
        const res = await this.repo.subirComprobante(id, file);
        if (res && res.ok && res.url) {
          const g = this.store.gastos.find((x) => x.id === id);
          if (g) g.comprobante = res.url;
          this.store.emit('gastos:update');
          closeModal();
          toast('Comprobante adjuntado correctamente', 'success');
        } else {
          toast('Error al subir el comprobante', 'error');
        }
      } catch {
        toast('Error al subir el comprobante', 'error');
      }
    };
    input.click();
  }

  /* ---------- ELIMINAR ---------- */
  private confirmarEliminarGasto(id: number): void {
    const g = this.store.gastos.find((x) => x.id === id);
    if (!g) return;
    modal(`
      <h2>Eliminar gasto</h2>
      <p style="font-size:14px;margin-bottom:18px;">¿Eliminar el gasto de <b>S/ ${g.monto.toFixed(2)}</b> en <b>${esc(g.categoria)}</b> · ${esc(g.proveedor)}?<br>
      <span style="color:var(--danger);font-size:13px;">También se eliminará el egreso de fondos asociado.</span></p>
      <div class="modal-footer">
        <button class="btn btn-outline" data-action="cerrar-modal">Cancelar</button>
        <button class="btn btn-primary" style="background:var(--danger);border-color:var(--danger);" data-action="do-eliminar-gasto" data-id="${id}">Sí, eliminar</button>
      </div>`, { narrow: true });
  }

  private async doEliminarGasto(id: number): Promise<void> {
    const res = await this.store.eliminarGasto(id) as { ok?: boolean } | null;
    if (res && res.ok === false) { toast('Error al eliminar', 'error'); return; }
    closeModal();
    toast('Gasto eliminado', 'success');
  }

  private confirmarEliminarIngreso(id: number): void {
    const m = (this.store.fondos.movimientos || []).find((x) => x.id === id);
    if (!m) return;
    modal(`
      <h2>Eliminar ingreso</h2>
      <p style="font-size:14px;margin-bottom:18px;">¿Eliminar el ingreso de <b>S/ ${m.monto.toFixed(2)}</b> — ${esc(m.descripcion || m.categoria)}?</p>
      <div class="modal-footer">
        <button class="btn btn-outline" data-action="cerrar-modal">Cancelar</button>
        <button class="btn btn-primary" style="background:var(--danger);border-color:var(--danger);" data-action="do-eliminar-ingreso" data-id="${id}">Sí, eliminar</button>
      </div>`, { narrow: true });
  }

  private async doEliminarIngreso(id: number): Promise<void> {
    const res = await this.store.eliminarFondoMovimiento(id) as { ok?: boolean } | null;
    if (res && res.ok === false) { toast('Error al eliminar', 'error'); return; }
    closeModal();
    toast('Ingreso eliminado', 'success');
  }

  /* ---------- HELPERS ---------- */
  private val(elId: string): string {
    return (document.getElementById(elId) as HTMLInputElement | HTMLSelectElement | null)?.value ?? '';
  }
}
