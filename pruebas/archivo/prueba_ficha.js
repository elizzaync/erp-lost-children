// Ficha con pestañas y avisos del Dashboard que aterrizan en el punto exacto.
...
const edge = spawn(EDGE, ["--headless=new","--disable-gpu","--no-sandbox",
  "--remote-debugging-port=9352","--user-data-dir="+path.join(SP,"edge-ficha"),
...
const aviso = (t) => evaluar(`(()=>{
  const b=[...document.querySelectorAll('button')].find(x=>x.innerText.includes(${JSON.stringify(t)}));
  return b? {texto:b.innerText.replace(/\\s+/g,' ').trim(), clic:()=>{}} : null;})()`);
const clicAviso = (t) => evaluar(`(()=>{
  const b=[...document.querySelectorAll('button')].find(x=>x.innerText.includes(${JSON.stringify(t)}));
  if(!b) return 'no encontrado'; b.click(); return 'ok';})()`);
const seccionActiva = () => evaluar(`(()=>{
  const b=[...document.querySelectorAll('button')]
    .filter(x=>/^(Datos|Documentos|Contratos)/.test(x.innerText.trim()))
    .filter(x=>Number(getComputedStyle(x).fontWeight)>=600);
  return b.length? b[0].innerText.trim().split('\\n')[0] : null;})()`);

(async()=>{
  let t=null;
  for(let i=0;i<40&&!t;i++){await dormir(500);
    try{const l=await fetch("http://127.0.0.1:9352/json/list").then(r=>r.json());
  ...
  console.log("\n1. Avisos del Dashboard");
  for (const t of ["Documentos por vencer","Evaluaciones sin iniciar","Contratos por renovar","Solicitudes de permiso"]) {
    const a = await aviso(t);
    console.log("   · " + (a ? a.texto.slice(0,96) : "NO ESTÁ"));
    check(!!a, "existe el aviso '" + t + "'");
  }

  console.log("\n2. 'Documentos por vencer' abre la pestaña Documentos de esa persona");
  check(await clicAviso("Documentos por vencer") === "ok", "el aviso es clicable");
  await dormir(1500);
  const t1 = await evaluar(`__texto()`);
  check(/elvis quispe/i.test(t1), "aterriza en la ficha de Elvis");
  check(await seccionActiva() === "Documentos", "y en la pestaña Documentos, no en Datos");
  const s1=await enviar("Page.captureScreenshot",{format:"png"});
  fs.writeFileSync(path.join(SP,"ficha-docs.png"),Buffer.from(s1.data,"base64"));

  console.log("\n3. 'Contratos por renovar' abre la pestaña Contratos");
  await evaluar(`(()=>{const b=[...document.querySelectorAll('button')].find(x=>x.innerText.trim().split('\\n')[0]==='Dashboard'); if(b)b.click();})()`);
  await dormir(1400);
  check(await clicAviso("Contratos por renovar") === "ok", "el aviso es clicable");
  await dormir(1500);
  check(await seccionActiva() === "Contratos", "aterriza en la pestaña Contratos");
  const s2=await enviar("Page.captureScreenshot",{format:"png"});
  fs.writeFileSync(path.join(SP,"ficha-contratos.png"),Buffer.from(s2.data,"base64"));

  console.log("\n4. Las pestañas se navegan a mano");
  for (const sec of ["Datos","Documentos","Contratos"]) {
    await evaluar(`(()=>{const b=[...document.querySelectorAll('button')].find(x=>x.innerText.trim().split('\\n')[0]===${JSON.stringify(sec)}); if(b)b.click();})()`);
    await dormir(600);
    check(await seccionActiva() === sec, "la pestaña " + sec + " se activa");
  }

  console.log("\n5. La ficha muestra datos reales del Legajo");
  await evaluar(`(()=>{const b=[...document.querySelectorAll('button')].find(x=>x.innerText.trim().split('\\n')[0]==='Datos'); if(b)b.click();})()`);
  await dormir(700);
  const t2 = await evaluar(`document.body.innerText.replace(/\\s+/g,' ')`);
  check(/docente de refuerzo/i.test(t2), "muestra el cargo real de la ficha");
  check(/educación/i.test(t2), "muestra el área real");
  const s3=await enviar("Page.captureScreenshot",{format:"png"});
  fs.writeFileSync(path.join(SP,"ficha-datos.png"),Buffer.from(s3.data,"base64"));
